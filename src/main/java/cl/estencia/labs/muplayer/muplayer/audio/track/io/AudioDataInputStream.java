package cl.estencia.labs.muplayer.muplayer.audio.track.io;
...
public class AudioDataInputStream extends InputStream {
    ...
    private static final long BUFF_SIZE = Math.round(Math.pow(1024, 2) * 10);
     ...
     */
    public AudioDataInputStream(InputStream stream, AudioFormat format, long length) {
        ...
        this.stream = new BufferedInputStream(stream, (int) Math.max(BUFF_SIZE,
                getStreamAvailableBytes(stream)));
        framePos = 0;
        markpos = 0;
    }

    private int getStreamAvailableBytes(InputStream in) {
        try {
            return in.available();
        } catch (IOException e) {
            return -1;
